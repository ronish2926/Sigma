...
class AllRides extends Constant implements Queries
{
    private $dtObject;
    private $actionType = "";
...
//        }
    ...
    public function retrieve()
    {
        ...
        if ($this->functionalityType == "retrieve_all_rides") {
            ...
            $queryDecorator = new QueryDecorator("SELECT ride_order.id as id,
captain.name as captain_name , captain.phone as captain_phone , captain.email as captain_email ,ride_order.status as sts, 
...
payment_method.name as payment_method_name  , ride_order.enable , rating.rating as ride_rating
            ...
            if (!empty($country_id)) {
                ...
                $queryDecorator = new QueryDecorator($queryDecorator->getQuery(), new CustomQuery("0", " INNER JOIN (
                    
                    SELECT country.id as country_id , ride_type.id as ride_type_id
                    ...
                    ) country_filter ON country_filter.country_id = '$country_id' AND country_filter.ride_type_id=ride_order.ride_type_id "));
            ...
            if (!empty($city_id)) {
                ...
                $queryDecorator = new QueryDecorator($queryDecorator->getQuery(), new CustomQuery("0", " INNER JOIN Location as location ON location.id = ride_type.place_id AND location.id = '$city_id'  "));
            ...
            /*. (!empty($country_id) ? " AND country.id = '$country_id' AND " : "")*/
            $queryDecorator = new QueryDecorator($queryDecorator->getQuery(), new CustomQuery("0", $where_con." ride_order.status = '$status' " ));
            ...
            ///Utility::Logger($queryDecoratorCount->getQuery());
