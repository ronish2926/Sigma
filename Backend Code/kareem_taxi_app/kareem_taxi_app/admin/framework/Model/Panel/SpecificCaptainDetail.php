...
class SpecificCaptainDetail extends Constant implements Queries
{
    private $dtObject;
    private $actionType = "";
    ...
    public function create()
    {
        ...
        $postdata = $this->postData;
         ...
         if ($this->functionalityType == "insert_picture") {
            /*Utility::DebugObject("this is file Data:  " . count($file));
            Utility::DebugObject($file['file']);*/
            ////$getdata = $this->getRandom;
            ///Utility::Logger("Random ID = "+$this->randomId);
...
orders.total_revenue , rating.avg_rating
            ...
            while ($row = rowRetriever($result)) {
                ...
                $currency_symbol = Utility::extractCharactersFromString($row['total_revenue']);
                array_push($listOfData, [
                    'id' => $row['cap_id'],
                    'name' => $row['name'],
                    'email' => $row['email'],
                    ...
                    'total_revenue' => $currency_symbol." ".$this->getTotalRevenue($row['total_revenue']),
...
ride_order.status , ride_order.enable , rating.rating as ride_rating
            ...
            while ($row = rowRetriever($result)) {
                ...
                $currency_symbol = Utility::extractCharactersFromString($row['fare']);
                array_push($listOfData, [
                    'id' => $row['id'],
                    ...
                    'waiting_charges' => $currency_symbol." ".$row['waiting_charges'],
                    'waiting_time' => $row['waiting_time']." min",
...
ride_order.status , ride_order.enable , rating.rating as ride_rating
            ...
            while ($row = rowRetriever($result)) {
                ...
                array_push($listOfData, [
                    ...
                    'city_name' => $row['city_name'],
                    'country_name' => $row['country_name'],
                    'waiting_charges' => $currency_symbol." ".['waiting_charges'],
                    'waiting_time' => $row['waiting_time']." min",
    ...
    public function update()
    {
        ...
        if ($this->functionalityType=="enable_captain"){
            ...
            $queryDecorator = new QueryDecorator( "UPDATE CaptainDocuments set status = '" . $enable . "'   where term_id = '" . $id . "'", new CustomQuery("0", " "));
            $run = queryRunner($queryDecorator->getQuery());
