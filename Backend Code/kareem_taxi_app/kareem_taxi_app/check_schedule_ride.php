 ...
 while ($row = rowRetrieverObject($result)) {
  ...
  $query_captain = "SELECT  captain.id , captain.name , captain.phone , ride_type.id as ride_type_id ,ride_type.name as ride_type_name,tracking.latitude,tracking.longitude,
    ...
    while ($row_captain = rowRetrieverObject($result_captain)) { 
          ...
          $ride_type_id=  $row_captain -> ride_type_id;
          
    }

    if (countRow($result_captain)>0) {

        update_firebase_order($ride_type_id,$order_id,$captain_id);
...
function update_firebase_order($ride_type_id,$order_id,$captain_id){
  ...
  car_detail.car_colour , car_detail.car_number_plate , tracking.latitude , tracking.longitude,captain.device_token , captain.phone , IF(rating.total_rating IS NOT NULL, rating.total_rating , '0') as total_rating
  ...
  while ($row = rowRetrieverObject($result)) { 
   ...
   //Retrieving pricing condition
  

  $query = "SELECT ride_meta.*
    FROM  RideMeta as ride_meta
  ...
  while ($row = rowRetrieverObject($result)) {
        $id = $row -> term_id;
        $ride_fare = $row -> price;
        $ride_fare_condition = $row -> distance_condition;
  }
  ...
  $riderObject -> fare = $ride_fare;
  $riderObject -> fare_condition = $ride_fare_condition;
