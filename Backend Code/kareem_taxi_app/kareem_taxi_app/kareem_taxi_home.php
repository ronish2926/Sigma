...
if (isset($_POST["comment"])) {
 ...
 $comment = escapeStringForDb($comment);
...
if (isset($_POST["waiting_time"]) && !empty($_POST["waiting_time"]) ) {
...
if ($object=="retrieve_ride_type") {
  ...
  orderProduct($ride_type_id,$user_id,$from,$from_address,$latitude,$longitude,$to,$to_address,$to_latitude,$to_longitude,$distance,$price,$time,$currency_symbol,$payment,$payment_customer_id,false
    ,$schedule_date);

}elseif ($object=="schedule_ride") {
  
  orderProduct($ride_type_id,$user_id,$from,$from_address,$latitude,$longitude,$to,$to_address,$to_latitude,$to_longitude,$distance,$price,$time,$currency_symbol,$payment,$payment_customer_id,true
    ,$schedule_date);
  ...
  accept_reject_ride($rider_id,$status);
...
}elseif ($object=="retrieve_all_ride_category_type") {
  # For searching specific restaurants
  retrieve_all_ride_category_type($place_id,$ride_type_id);
...
}elseif ($object=="add_waiting_charges") {
  
  addWaitingChargesFee($order_id,$user_id, $captain_id,$waiting_time);

}elseif ($object=="send_notification_waiting_charges") {
  
  sendNotificationForWaitingCharges($order_id,$user_id);
...
function retrieve_ride_type($userlatitude,$userlongitude,$city,$user_id) {
  ...
  WHERE FIND_IN_SET(location.name,'$city') AND location.enable = '0' ";
  ...
  $query = "SELECT DISTINCT payment_type.payment_method_id as id , payment_method.name ,  payment_method.tag , payment_method.picture , 
  ...
  WHERE FIND_IN_SET(location.name,'$city') AND location.enable = '0' AND location.country_id = country.id ";
...
function find_nearest_ride_distance($userlatitude,$userlongitude,$ride_type_id) {
  ...
  ///var_dump(rowRetrieverObject($result));
  
  if(countRow($result)<=0){
      
      $general -> code ="206";
      $general -> message ="No captain available right now";
  ...
  ///echo "Url = ".$map_box_url." Data = ".$post_data;
...
function retrieve_all_ride_category_type($place_id,$ride_type_id) 
{
  ...
  $table = $GLOBALS['table_name_ride_type'];
  $query = "SELECT  id , name , tagline , picture FROM $table 
      WHERE enable = '0' AND place_id = '$place_id' AND ride_category = '$ride_type_id' ";
...
function orderProduct($ride_type_id,$user_id,$from,$from_address,$from_latitude,$from_longitude,$to,$to_address,$to_latitude,$to_longitude,$distance,$price,$time,$currency_symbol,$payment,$payment_customer_id,$schedule,$schedule_date) 
{
  ...
  $table = $GLOBALS['table_name_ride_order'];  
    ...
    if (!$schedule) {
      ...
      $query = " SELECT  captain.id , captain.name , captain.phone , ride_type.id as ride_type_id ,ride_type.name as ride_type_name,tracking.latitude,tracking.longitude,
       ...
       (SELECT COUNT(*) FROM RideOrder WHERE captain_id = captain.id AND status = '1') as order_count
      FROM Tracking as tracking
      INNER JOIN Captain as captain ON captain.id = tracking.term_id AND captain.enable = '0' AND captain.status = '0' AND captain.live = '0' AND captain.ride_type_id = '$ride_type_id'
      INNER JOIN RideType as ride_type ON ride_type.id = captain.ride_type_id AND ride_type.enable = '0' 
      INNER JOIN Location as location ON location.id = ride_type.place_id AND location.enable = '0' 
      INNER JOIN Country as country ON country.id = location.country_id AND country.enable = '0' 
      HAVING distance <= '2' AND order_count <= '0' ";
    ...
    $query = " SELECT waiting_charges.initial_charges FROM RideType as ride_type
    INNER JOIN RideTypeWaitingCharges as waiting_charges ON waiting_charges.term_id = ride_type.id
    WHERE ride_type.id = '$ride_type_id' AND ride_type.enable='0' ";
    ...
    $total = extract_numbers($price)[0] + /*$initial_charges*/0;
    $price = $currency_symbol." ".$total;
        
    if ($payment == "1") {

        $query = "INSERT INTO $table
        (ride_type_id,user_id,captain_id,from_address_name,from_address,from_latitude,from_longitude,to_address_name,to_address,to_latitude,to_longitude,distance,price,trip_time,payment,status,customer_payment_id
        ,company_percentage)
       VALUES
        ('$ride_type_id','$user_id','$captain_id','$from','$from_address','$from_latitude','$from_longitude','$to','$to_address','$to_latitude','$to_longitude','$distance','$price','$time','$payment'
        ,'$status','$payment_customer_id','$company_percentage') ";
       ...
       $order_id=findLastInsertId();    
        ...
        if ($result != null) {
             ...
             if($schedule){
                ...
                $query = "UPDATE $table SET schedule_date_created = '$schedule_date' WHERE id = '$order_id'";
                $result=queryRunner($query);
     ...
     $productOrder = chargePayment($order_id,$payment_customer_id,$currency_symbol,extract_numbers($price));
       ...
       if ($productOrder -> code == "202") {
        ...
        ,company_percentage)
       VALUES
        ('$ride_type_id','$user_id','$captain_id','$from_address_name','$from_address','$from_latitude','$from_longitude','$to_address_name','$to_address','$to_latitude','$to_longitude','$distance','$price','$time','$payment'
        ,'$status','$payment_customer_id','$company_percentage') ";
...
function addRecordIntoFirebase($order_id,$user_id,$captain_id,$from_latitude,$from_longitude,$to_latitude,$to_longitude,$company_percentage,$price) 
{
  ...
  car_detail.car_colour , car_detail.car_number_plate , tracking.latitude , tracking.longitude , captain.phone , captain.device_token ,   IF(rating.total_rating IS NOT NULL, rating.total_rating , '0') as total_rating
  ...
  LEFT JOIN (
   ...
   SELECT captain_id as id , AVG(rating) as total_rating FROM Rating 
  
  ) as rating ON rating.id = captain.id
  ...
  $query = "SELECT user.id , concat(user.first_name,' ',user.last_name) as user_name , user.profile_picture as user_picture  , user.phone
  ...
  $query="SELECT ride_order.from_address_name , ride_order.from_address , ride_order.to_address_name , ride_order.to_address ,
  payment_method.name as payment_method_name, payment_method.picture as payment_method_picture ,
  ride_type.name as ride_type_name , ride_type.picture as ride_type_picture
  ...
  $result=queryRunner($query);
  while ($row = rowRetrieverObject($result)) { 
        $from = $row -> from_address_name;
        $from_address = $row -> from_address;
        $to = $row -> to_address_name;
        $to_address = $row -> to_address;
        $payment_method_name = $row -> payment_method_name;
        $payment_method_picture = $row -> payment_method_picture;
        $ride_type_name = $row -> ride_type_name;
        $ride_type_picture = $row -> ride_type_picture;
  }
  ...
  $riderObject -> rider_phone = $phone;
  $riderObject -> rating = $total_rating;
  ...
  $userObject -> user_phone = $user_phone;
  ...
  $destinationObject -> from = $from;
  $destinationObject -> from_address = $from_address;
  
  $destinationObject -> to = $to;
  $destinationObject -> to_address = $to_address;
  
  $destinationObject -> ride_type_image = $ride_type_picture;
  $destinationObject -> ride_type_name = $ride_type_name;
  
  $destinationObject -> payment_type_image = $payment_method_picture;
  $destinationObject -> payment_type_name = $payment_method_name;
...
function addWaitingChargesFee($order_id,$user_id, $captain_id,$waiting_time) 
{
  ...
  $query = " SELECT ride_order.price as order_price , currency.currency_symbol , ride_order.payment , ride_order.customer_payment_id as token , waiting_charges.price as item_price , waiting_charges.time_condition as time_span
      ...
      INNER JOIN Configuration as currency ON currency.country_id = country.id
      WHERE ride_order.id = '$order_id' AND ride_order.enable='0' ";
  ...
  if($waiting_time > $time_span){
      
      $total_price = ($waiting_time / $time_span) * $price;
      $old_price = extract_numbers($order_price);
      
      if($token!=null || $token!=""){
       $productOrder = chargePayment($order_id,$token,$currency_symbol,$total_price); 
      }
      $total_price = $old_price[0] + $total_price;
      
      $productOrder -> code = "202";
      //echo "Total Values = ".$total_price." Old Value = ".$old_price;
      
      if ($productOrder -> code == "202") {
          $price_with_currency = $currency_symbol." ".$total_price;
          $query = "UPDATE $table SET price = '$price_with_currency'
          WHERE id = '$order_id'  ";
          ...
          $productOrder -> code ="202";
          $productOrder -> message = "Order Successfully";
          $productOrder -> waiting_fee = $currency_symbol." ".$total_price;
          $productOrder -> payment_type = $payment;
      
      }
      else{
          
          /*$productOrder -> code ="206";
          $productOrder -> message = "Failed to adding fee";*/
...
function sendNotificationForWaitingCharges($order_id,$user_id) 
{
  ...
  while ($row = rowRetrieverObject($result)) { 
   ...
   sendNotificationToUser($device_token,false);
...
function addPaymentCards($user_id,$card_number,$card_company,$expiry_month,$expiry_year,$stripe_customer_id) 
{
 ...
 \Stripe\Stripe::setApiKey($public_token);
  ...
  //========For Testing purpose , uncomment this line==========

  //////\Stripe\Stripe::setApiKey($private_token);  
...
function addDocumentsToRider($captain_id,$document_type,$pictures) {
  ...
  for( $i = 0; $i<count($pictures); $i++ ) {
    ...
    file_put_contents('admin/public/uploads/driver_detail_pic/'.$pictureName, $pictureData);
...
function loginRider($email,$pass)
{
  ...
  if ($count>0) {
    ...
    while ($row = rowRetriever($result)) {
     ...
     $user -> deviceToken=$data[0]['device_token'];
...
function registerRider($email,$pass,$fName,$avatar,$deviceId,$phone,$car_brand,$car_name,$car_colour,$car_number_plate,$address,$latitude,$longitude,$place_id,$car_category_id)
{
  ...
  $query = "INSERT INTO $tableUser (name,email,password,phone,device_token,address,latitude,longitude,place_id,ride_type_id) VALUES ('$fName','$email','$pass','$phone','$userToken','$address','$latitude','$longitude','$place_id','$car_category_id')";
  $result=queryRunner($query);
  $count=countRow($result);
    ...
    if ($result != null) {
         ...
         file_put_contents('admin/public/uploads/captainpic/'.$userId.'.png', $pictureData);
...
function sendNotificationToUser($device_token,$isCaptain){
     ...
     if ($isCaptain) {
        $message = "You received an order";
         ...
         $message = "Captain reached your destination";
     }
     ...
     $content = array(
      ...
      $fields = array(
            ...
            'include_external_user_ids' => array($device_token),
...
function register($email,$pass,$fName,$lName,$avatar,$userType,$uid,$deviceId,$phone)
{
  ...
  if ($userType=="native") {
      $query = "INSERT INTO $tableUser (first_name,last_name,email,password,phone,login_type,device_token) VALUES ('$fName','$lName','$email','$pass','$phone','$userType','$userToken')";
    }else {
      $query = "INSERT INTO $tableUser (first_name,last_name,email,password,phone,login_type,uid,device_token) VALUES ('$fName','$lName','$email','$email','$phone','$userType','$uid','$userToken')";
      $pass = $email;
    } 
